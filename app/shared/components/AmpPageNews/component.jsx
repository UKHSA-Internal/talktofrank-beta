...
const AmpPageNews = ({pageData, location, siteSettings}) => {
  return (
    <React.Fragment>
      ...
      <Main>
        <Accent className='accent--shallow spacing-top--single' modifier='wrapper--constant'>
          <Grid>
            {pageData.fields.image && <GridCol className='col-12 list-offset'>
              <Article isAmp {...this.props}/>
            </GridCol>}
            {!pageData.fields.image && <GridCol className='col-12 col-sm-8 offset-md-2 spacing-top--single'>
              {pageData.date && <Time time={pageData.dateFormatted} dateTime={pageData.date}/>}
              <Heading type='h1' text={pageData.fields.title} className='h3 spacing-bottom--large'/>
            </GridCol>}
